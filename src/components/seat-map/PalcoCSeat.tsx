...
interface PalcoCSeatNodeProps {
  seat: PalcoCSeat;
  onToggle: (seat: PalcoCSeat) => void;
}
...
function PalcoCSeatComponent({ seat, onToggle }: PalcoCSeatNodeProps) {
  ...
  return (
    ...
    >
      ...
      <g opacity={opacity} style={{ transition: 'opacity 160ms ease' }}>
        <rect
          x={-PALCO_C_SEAT_WIDTH / 2 + 2}
          y={backY}
          width={PALCO_C_SEAT_WIDTH - 4}
          height={backH}
          rx={3}
          ry={3}
          fill={fill}
        ...
        />
        <rect
          className="palco-c-seat-rect"
          x={-PALCO_C_SEAT_WIDTH / 2}
          y={seatBodyY}
          width={PALCO_C_SEAT_WIDTH}
          height={seatBodyH}
          rx={PALCO_C_SEAT_RADIUS}
          ry={PALCO_C_SEAT_RADIUS}
          fill={fill}
          stroke={stroke}
          strokeWidth={1}
        />
      ...
      {isSelected && (
        <g pointerEvents="none">
          <circle
            cx={PALCO_C_SEAT_WIDTH / 2 - 2}
            cy={-PALCO_C_SEAT_HEIGHT / 2 + 2}
            r={4}
          ...
          />
          <path
            d={`M ${PALCO_C_SEAT_WIDTH / 2 - 4} ${-PALCO_C_SEAT_HEIGHT / 2 + 2} L ${PALCO_C_SEAT_WIDTH / 2 - 2.5} ${-PALCO_C_SEAT_HEIGHT / 2 + 3.4} L ${PALCO_C_SEAT_WIDTH / 2 - 1} ${-PALCO_C_SEAT_HEIGHT / 2 + 0.8}`}
            stroke="#3A1F00"
            strokeWidth={1.2}
            strokeLinecap="round"
      ...
      {isDisabled && (
        <g pointerEvents="none" opacity={0.55}>
          <line
            x1={-PALCO_C_SEAT_WIDTH / 2 + 6}
            y1={seatBodyY + 4}
            x2={PALCO_C_SEAT_WIDTH / 2 - 6}
...
export const PalcoCSeatNode = memo(PalcoCSeatComponent);